...
//' @export
// [[Rcpp::export]]
Rcpp::List two_species_mosquito_population_model(int start_time, int end,
  ...
  // Declaring Additional Parameters
    ...
    // Species One
    double K_1; // ecological carrying capacity of the environment
    ...
    // Species Two
    double K_2; // ecological carrying capacity of the environment
      ...
      if (j == 0) {
        ...
        if (rainfall_relationship_species_one == "mean") {
          ...
          if (t <= tau_with_dt_1) {
            ...
            K_1 = (1.0 + ((1.0 / tau_with_dt_1) * rFsum_1));
            k_output_1[t] = K_1;
        ...
        if (mortality_density_function_species_one == "linear") {
          muE_1 = muE0_1 * (1 + ((E_1 + L_1) / (K_1)));
          muL_1 = muL0_1 * (1 + lambda_1 * (((E_1 + L_1) / (K_1))));
        }
      ...
      if (j == 1) {
        ...
        if (rainfall_relationship_species_two == "mean") {
          ...
          if (t <= tau_with_dt_2) {
            ...
            K_2 = (1.0 + ((1.0 / tau_with_dt_2) * rFsum_2));
            k_output_2[t] = K_2;
        ...
        // Setting the density dependent function regulating larval mortality
        if (mortality_density_function_species_one == "linear") {
          muE_2 = muE0_2 * (1 + ((E_2 + L_2) / (K_2)));
          muL_2 = muL0_2 * (1 + lambda_2 * (((E_2 + L_2) / (K_2))));
        }
