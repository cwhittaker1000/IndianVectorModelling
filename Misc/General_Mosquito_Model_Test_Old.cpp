...
//' @export
// [[Rcpp::export]]
Rcpp::List general_mosquito_population_model_test_old(int start_time, int end,
  ...
  // Initial code to do all the calculations for the exponential relationship just once, as they're computationally expensive
  std::vector<double> Offset_Period_Exponential_Static;
  std::vector<double> Offset_Period_Exponential_Rainfall;

  double calc_placeholder_static;
  double calc_placeholder_rainfall;
  double temp_tau_static_prior = tau_with_dt_static;
  double temp_tau_rainfall_prior = tau_with_dt_rain;

  for (int i = 0; i <= (offset + end_time); i++) {

    calc_placeholder_static = -((offset + end_time) - i)/temp_tau_static_prior;
    calc_placeholder_rainfall = -((offset + end_time) - i)/temp_tau_rainfall_prior;
    ...
    // Offset_Period_Exponential_Static.emplace_back(exp(calc_placeholder_static));
    // Offset_Period_Exponential_Rainfall.emplace_back(exp(calc_placeholder_rainfall));
  ...
  for (int p = tau_with_dt_static; p < (offset + start_time); p++) {
    ...
    if (rainfall_relationship == "mean") {
      ...
      double calc;
      for (int r = 0; r <= p; r++) {
        calc = Offset_Period_Exponential_Static[r];
    ...
    if (rainfall_relationship == "mean") {
      ...
      if (rainfall_effect == "raw") {
        ...
        for (int r = 0; r <= t + offset; r++) { // need to decide whether to start at beginning of rain or t = 0 ??
          calc = Offset_Period_Exponential_Rainfall[r];
  ...
  return Rcpp::List::create(Rcpp::Named("E_Output") = E_output,
                            ...
                            Rcpp::Named("new_exp_static") = Offset_Period_Exponential_Static,
                            Rcpp::Named("new_exp_rainfall") = Offset_Period_Exponential_Rainfall);
}
