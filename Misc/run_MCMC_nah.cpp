...
//' @export
// [[Rcpp::export]]
Rcpp::List runMCMC_nah(int start_sd_adaptation, // Time to start SD adaptation - relates to SD adaptation
  ...
  for (int i = 0; i < number_of_iterations; i++) {
    ...
    for (int j = 0; j < parameters_to_be_fitted.size(); j++) { // NOTE: MEANS THAT THE ORDER MATTERS CURRENTLY RE FLEXIBILITY IN TERMS OF WHICH PARAMETERS ARE FITTED
      ...
      if (i >= start_sd_adaptation & i < end_sd_adaptation) {
        sd_proposals[j] = proposal_SD_adapter(sd_proposals[j], acceptance_ratio[j], current_acceptance_ratio[j], max_sd[j]);
