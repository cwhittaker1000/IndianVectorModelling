...
double prior_seq_proposals(Rcpp::NumericVector parameter_values, Rcpp::String prior_choice, Rcpp::LogicalVector fitted_yn) {
  ...
  if (prior_choice == "informative") {
    ...
    if (fitted_yn["scaling_factor"]) {
      res = res + R::dunif(parameter_values["scaling_factor"], 1, 50, TRUE); //pop_frac BETA DISTRIBUTION USED BASED ON MRR DATA
    }
    if (fitted_yn["K_static"]) {
      res = res + R::dunif(parameter_values["K_static"], 0, 200, TRUE); //z Uniform distribution used
    }
...
//' @export
// [[Rcpp::export]]
double likelihood_function(int N, std::vector <double> rainfall, std::vector <int> obsData, int number_of_datapoints, int data_timeframe,
                           Rcpp::NumericVector fitted_parameters, Rcpp::NumericVector static_parameters, Rcpp::String density_function,
                           double sampling_point, Rcpp::StringVector month_vector, Rcpp::String rainfall_relationship, Rcpp::String rainfall_effect) {

  double loglikelihood = min_output_particle_filter(N, rainfall, obsData, number_of_datapoints, data_timeframe, fitted_parameters,
                                                    static_parameters, density_function, sampling_point, month_vector, rainfall_relationship, rainfall_effect);
...
double posterior_seq_proposals(int N, std::vector <double> rainfall, std::vector <int> obsData, int number_of_datapoints,
                               ...
                               Rcpp::String density_function, double parameter_value, int parameter_index, Rcpp::String prior_choice,
                               Rcpp::LogicalVector fitted_yn, double sampling_point, Rcpp::StringVector month_vector, Rcpp::String rainfall_relationship, Rcpp::String rainfall_effect) {
  ...
  if (prior_seq_proposals(fitted_parameters, prior_choice, fitted_yn) < -5000) {
    ...
    posterior_likelihood = likelihood_function(N, rainfall, obsData, number_of_datapoints, data_timeframe, fitted_parameters, static_parameters,
                                               density_function, sampling_point, month_vector, rainfall_relationship, rainfall_effect)
