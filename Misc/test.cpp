...
//' @export
// [[Rcpp::export]]
Rcpp::List tester_runMCMC(int start_sd_adaptation, // Time to start SD adaptation - relates to SD adaptation
  ...
  for (int i = 0; i < number_of_iterations; i++) {
    ...
    for (int j = 0; j < Rcpp::sum(fitted_yn); j++) { // NOTE: MEANS THAT THE ORDER MATTERS CURRENTLY RE FLEXIBILITY IN TERMS OF WHICH PARAMETERS ARE FITTED
      ...
      double proposed_parameter_value = proposal_function(sd_proposals[index], current_parameter_value);
      ...
      if (i >= start_sd_adaptation & i < end_sd_adaptation) {
        sd_proposals[index] = proposal_SD_adapter(sd_proposals[index], acceptance_ratio[index], current_acceptance_ratio[j], max_sd[index]);
