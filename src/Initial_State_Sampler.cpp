...
std::vector <int> test_initial_state_sample(Rcpp::NumericVector fitted_parameters,
  ...
  double dE = 1/fitted_parameters[0]; double dL = 1/fitted_parameters[1];
  double dP = 1/fitted_parameters[2]; double muE0 = fitted_parameters[3];
  double muL0 = fitted_parameters[4]; double muP = fitted_parameters[5];
  double muM = fitted_parameters[6]; double lambda = fitted_parameters[7];
  double tau = fitted_parameters[8];  double beta = fitted_parameters[9];
  double z = fitted_parameters[13];   double scaled_static_K = fitted_parameters[12] * fitted_parameters[14]; // fitted_parameters[14] = K_static, fitted_parameters[12] = scaling factor
  ...
  // double overdisp = fitted_parameters[10] // Not needed for initial states calculation
  // double pop_frac = fitted_parameters[11] // Not needed for initial states calculation
  // double dt = static_parameters[0]; // Not needed for initial states calculation
