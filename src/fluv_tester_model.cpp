...
Rcpp::List mosquito_population_model_fluv(int start_time, int end, Rcpp::NumericVector fitted_parameters,
  ...
  double scaling_factor = fitted_parameters[12]; double K_max = fitted_parameters[15], hill_1 = fitted_parameters[16], hill_2 = fitted_parameters[17];
  ...
  while (t < end_time) {
    ...
    if (mortality_density_function == "power") {
      muE = muE0 * (1 + pow(((E + L) / (K_static - K)), dd_pow));
      muL = muL0 * (1 + (lambda * pow(((E + L) / (K_static - K)), dd_pow)));
    }
    else if (mortality_density_function == "exponential") {
      muE = muE0 * exp(((E + L) / (K_static - K)));
      muL = muL0 * exp(lambda * ((E + L) / (K_static - K)));
    }
    else if (mortality_density_function == "linear") {
      muE = muE0 * (1 + ((E + L) / (K_static - K)));
      muL = muL0 * (1 + lambda * ((E + L) / (K_static - K)));
    }
