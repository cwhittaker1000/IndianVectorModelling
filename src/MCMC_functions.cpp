...
double prior(Rcpp::NumericVector parameter_values, Rcpp::LogicalVector fitted_yn, Rcpp::String likelihood_choice) {
  ...
  if (fitted_yn["tau_rain"]) {
    res = res + R::dnorm4(parameter_values["tau_rain"], 4, 0.7653, TRUE); // CONSIDER CHANGING THIS
  }
  ...
  if (fitted_yn["K_Max_Rainfall"]) {
    res = res + R::dunif(parameter_values["K_Max_Rainfall"], 1, 500, TRUE); //pop_frac BETA DISTRIBUTION USED BASED ON MRR DATA
  }
  if (fitted_yn["Hill_Rain_1"]) {
    res = res + R::dunif(parameter_values["Hill_Rain_1"], 1, 500, TRUE); //pop_frac BETA DISTRIBUTION USED BASED ON MRR DATA
  }
  if (fitted_yn["Hill_Rain_2"]) {
    res = res + R::dunif(parameter_values["Hill_Rain_2"], 1, 500, TRUE); //pop_frac BETA DISTRIBUTION USED BASED ON MRR DATA
  }
...
double likelihood_function(int N, std::vector <double> rainfall, std::vector <int> obsData,
  ...
  double loglikelihood =   min_output_particle_filter(N, rainfall, obsData, fitted_parameters, static_parameters,
...
double posterior_joint_proposals(int N, std::vector <double> rainfall, std::vector <int> obsData,
  ...
  if (prior(fitted_parameters, fitted_yn, likelihood_choice) < -5000) {
    ...
    posterior_likelihood = likelihood_function(N, rainfall, obsData,
...
double old_prior(Rcpp::NumericVector parameter_values, Rcpp::String prior_choice, Rcpp::LogicalVector fitted_yn, Rcpp::String likelihood_choice) {
